...
INVOICE_PNG = "png"
_MONTH_KEYS = {
    "thang", "month", "thang_hdon", "thanghdon", "thang_hoa_don", "thanghoadon",
    "ky", "ky_hdon", "kyhdon", "ky_hoa_don", "kyhoadon",
}
_YEAR_KEYS = {"nam", "year", "nam_hdon", "namhdon", "nam_hoa_don", "namhoadon"}
_ATTACHMENT_KEY_TOKENS = (
    "pdf", "png", "file", "tep", "url", "uri", "link", "download", "attachment",
    "dinhkem", "dinh_kem", "hoadon", "hoa_don", "invoice", "image", "hinhanh",
    "hinh_anh", "path", "src", "href", "document", "chungtu", "chung_tu", "hddt",
    "viewer", "resource",
)
_INVOICE_TEXT_TOKENS = ("hóa đơn", "hoa don", "tiền điện", "tien dien", "invoice", "bill")
...
def is_invoice_notification(record: Any) -> bool:
    ...
    if not isinstance(record, dict):
        return False
    category = str(
        record.get("notificationType") or record.get("loai") or record.get("type") or ""
    ).upper()
    ...
    text = " ".join(
        ...
        for key in (
            "title", "summary", "strTieuDe", "TieuDe", "tieuDe", "strNoiDung",
            "noiDung", "content",
        )
    ).casefold()
...
def iter_attachment_candidates(record: Any) -> Iterator[tuple[str, str]]:
    ...
    def walk(value: Any, key_hint: str = "") -> Iterator[tuple[str, str]]:
        if isinstance(value, dict):
            ...
            items.sort(key=lambda item: 0 if _attachment_key(str(item[0])) else 1)
        ...
        if key_relevant and len(text) <= 4096 and not any(ch.isspace() for ch in text) and (
            text.startswith(("/", "./", "../"))
            or ".pdf" in lowered or ".png" in lowered or "download" in lowered
            or "invoice" in lowered or "hoadon" in lowered or "hddt" in lowered
